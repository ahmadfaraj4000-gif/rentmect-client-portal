...
export default function BirthdayInput({
  ...
  return (
    <fieldset className="birthday-input" aria-describedby={statusId} disabled={disabled}>
      <legend>Date of birth</legend>
      <div className="birthday-segments" role="group" aria-label="Date of birth, month day and year">
        <label><span>MM</span><input ref={monthRef} name="bday-month" type="text" inputMode="numeric" autoComplete="bday-month" pattern="[0-9]*" placeholder="MM" value={parts.month} onChange={(event) => updatePart('month', event.target.value)} aria-label="Birth month, two digits" autoFocus={autoFocus} /></label>
        <span className="birthday-divider" aria-hidden="true">/</span>
        <label><span>DD</span><input ref={dayRef} name="bday-day" type="text" inputMode="numeric" autoComplete="bday-day" pattern="[0-9]*" placeholder="DD" value={parts.day} onChange={(event) => updatePart('day', event.target.value)} onKeyDown={(event) => handleKeyDown(event, 'day')} aria-label="Birth day, two digits" /></label>
        <span className="birthday-divider" aria-hidden="true">/</span>
        <label className="birthday-year"><span>YYYY</span><input ref={yearRef} name="bday-year" type="text" inputMode="numeric" autoComplete="bday-year" pattern="[0-9]*" placeholder="YYYY" value={parts.year} onChange={(event) => updatePart('year', event.target.value)} onKeyDown={(event) => handleKeyDown(event, 'year')} aria-label="Birth year, four digits" /></label>
      </div>
      ...
      {!disabled && <label className={`birthday-confirmation ${!eligible ? 'disabled' : ''}`}>
        <input type="checkbox" checked={Boolean(confirmed && eligible)} disabled={!eligible} onChange={(event) => onConfirmedChange(event.target.checked)} />
